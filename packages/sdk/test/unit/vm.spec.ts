...
async function getVm(project: Project, { delay = 0 }: { delay?: number } = {}) {
  ...
  return new VM(channel.port1, {
    previewOrigin: project.template === 'node' ? undefined : 'https://test.khulnasoft.io',
  });
}
...
describe('vm.editor', () => {
  ...
  test('can change open files', async () => {
    const files = { 'README.md': 'Hi.', '.khulnasoftrc': '{}' };
    const vm = await getVm(getTestProject({ files }));
...
describe('vm.preview', () => {
  test('has origin', async () => {
    const vm = await getVm(getTestProject());
    expect(vm.preview.origin).toBe('https://test.khulnasoft.io');
  });
  ...
  test('can set preview URL', async () => {
    ...
    await expect(vm.preview.getUrl()).resolves.toBe('https://test.khulnasoft.io/about');
  });
