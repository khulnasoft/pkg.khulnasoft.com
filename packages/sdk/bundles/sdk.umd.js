(function(global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.khulnaSoftSDK = factory());
})(this, function() {
  "use strict";
  const CONNECT_INTERVAL = 500;
  const CONNECT_MAX_ATTEMPTS = 20;
  const DEFAULT_FRAME_HEIGHT = 300;
  const DEFAULT_ORIGIN = "https://khulnasoft.com";
  ...
  class VM {
    constructor(port, config) {
      ...
      this.preview = {
         ...
         *
         * @see https://developer.khulnasoft.com/guides/user-guide/available-environments
         */
        origin: "",
        /**
      ...
      this.pending = new Promise((resolve, reject) => {
        ...
        const interval = window.setInterval(() => {
          ...
          if (attempts >= CONNECT_MAX_ATTEMPTS) {
            cleanup();
            reject("Timeout: Unable to establish a connection with the khulnaSoft VM");
  ...
  function createProjectForm({
    ...
    const projectFields = [
      ...
      { name: "project[template]", value: template ?? "javascript" },
      {
        ...
        value: dependencies ? JSON.stringify(dependencies) : void 0,
        condition: dependencies && template !== "node"
      },
      {
        name: "project[settings]",
        value: settings ? JSON.stringify(settings) : void 0,
        condition: !!settings
      }
    ];
    const inputs = projectFields.filter((field) => field.condition ?? true).map((field) => createHiddenInput(field.name, field.value));
...
<body>
  ...
  function embedProject(elementOrId, project, options) {
    const frame = _embed(elementOrId, options);
  ...
  function _embed(elementOrId, options) {
