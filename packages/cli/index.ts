...
import type { Comment } from "@pkg-khulnasoft-com/utils";
import {
  abbreviateCommitHash,
  extractOwnerAndRepo,
  extractRepository,
} from "@pkg-khulnasoft-com/utils";
...
async function verifyCompactMode(packageName: string) {
  ...
  try {
    ...
    throw new Error(
      `pkg-khulnasoft-com cannot resolve ${packageName} from npm. --compact flag depends on the package being available in npm.
...
See https://docs.npmjs.com/cli/v10/configuring-npm/package-json#repository for details.`;
  ...
  if (!repository) {
    throw new Error(
      `pkg-khulnasoft-com cannot extract the repository link from the ${packageName} manifest. --compact flag requires the link to be present.
${instruction}`,
    );
  }
  ...
  if (!match) {
    throw new Error(
      `pkg-khulnasoft-com cannot extract the owner and repo names from the ${packageName} repository link: ${repository}. --compact flag requires these names.
${instruction}`,
    );
  }
}
