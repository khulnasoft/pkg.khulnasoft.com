...
import type { PackageManager } from "@pkg-khulnasoft-com/utils";
...
export default eventHandler(async (event) => {
  ...
  if (!(await workflowsBucket.hasItem(key))) {
    throw createError({
      statusCode: 401,
      message:
        "Try publishing from a github workflow! Also make sure you install https://github.com/apps/pkg-khulnasoft-com GitHub app on the repo",
    });
  }
