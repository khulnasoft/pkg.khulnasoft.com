...
<p align="center">
  ...
  <a href="https://valtio.pmnd.rs/"><img src="https://blog.stackblitz.com/posts/pkg-khulnasoft-com/valtio.svg" height="40"></a>
...
First [install the GitHub Application](https://github.com/apps/pkg-khulnasoft-com).
...
After installing on your repository, you can run `npx pkg-khulnasoft-com publish` in your workflows and then you have continuous releases!

```sh
npm install --save-dev pkg-khulnasoft-com # or `npx pkg-khulnasoft-com publish`
```

For workspaces and monorepos:

```sh
npx pkg-khulnasoft-com publish './packages/A' './packages/B' # or `npx pkg-khulnasoft-com publish './packages/*'`
```

> [!IMPORTANT]
> Make sure the pkg-khulnasoft-com command runs only once in a defined workflow (that's part of how it avoids spam)! So instead of multiple times running pkg-khulnasoft-com for each workspace package, the command should be run one time for all the desired packages using the pattern above.
...
```sh
npx pkg-khulnasoft-com publish './packages/A' --template './examples/*'
```
...
```sh
npx pkg-khulnasoft-com publish --compact './packages/A' './packages/B'
```
...
```sh
npx pkg-khulnasoft-com publish --bin
```

With `--bin`:

```sh
npx https://pkg.khulnasoft.com/pkg-khulnasoft-com@a832a55
```

Without `--bin`:

```sh
npm i https://pkg.khulnasoft.com/pkg-khulnasoft-com@a832a55
```

You can control publishing comments with `--comment`:

```sh
npx pkg-khulnasoft-com publish --comment=update # default
```
...
jobs:
  build:
    ...
    steps:
      ...
      - run: pnpm dlx pkg-khulnasoft-com publish
```
...
- name: Publish packages
  run: npx pkg-khulnasoft-com publish --json output.json --comment=off
```
